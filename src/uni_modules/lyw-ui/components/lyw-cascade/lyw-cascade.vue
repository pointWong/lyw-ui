<template>
  <view :class="['lyw-dropdown', disabled ? 'disabled' : '']" :style="{ ...styles }">
    <view :class="['lyw-dropdown-selected']" @click.stop="openPopup" :style="{ height: styles.height, lineHeight: styles.height }">
      <text class="selected current oClamp" v-if="setSelected().text && !locate">{{ setSelected().text || current }}</text>
