<template>
  <view :class="['lyw-dropdown', disabled ? 'disabled' : '']" @click.stop="openPopup" :style="styles">
    ...
    <text class="selected placeholder" v-else>{{ placeHolder || '请选择' }}</text>
  </view>
...
const change = async (e) => {
  ...
  if (pickViewValues.value[1] !== undefined) {
...
const openPopup = () => {
  ...
  if (!setSelected.value) {
    ...
    const yIndex = years.value.findIndex((y) => y == year)
    const mIndex = months.value.findIndex((m) => m == month)
    const dIndex = days.value.findIndex((d) => d == day)
    pickViewValues.value = [yIndex, mIndex, dIndex]
  }
  popup.value.open()
}
